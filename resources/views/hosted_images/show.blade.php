<x-app-layout>
    ...
    @php
        ...
        // Checking if user has liked image
        $liked = $hostedImage->likes()->where('user_id', auth()->id())->exists();
    ...
    <section class="grid w-full p-6 gap-y-6 grid-cols-1 sm:grid-cols-[1fr,33%,1fr]  sm:gap-x-6">
        ...
        <div class="col-start-1 sm:col-start-2 w-full mx-auto">
            <div class="grid gap-3 p-6 sm:p-6 rounded-md bg-zinc-800 overflow-hidden">
                ...
                <div>
                    ...
                    @if ($editing)
                        <form method="POST" action="{{ route('uploads.update', $hostedImage->id) }}">
                            @csrf
                            @method('PUT')
                            ...
                            <input 
                                type="text" 
                                name="file_renamed"
                                value="{{ old('file_renamed', $hostedImage->file_renamed) }}"
                    ...
                    @else
                        <h1>{{ $hostedImage->file_renamed }}</h1>
                    @endif
                ...
                <div id="img_container" class="relative aspect-[3/4] max-h-[475px] overflow-hidden rounded-md w-full bg-zinc-900">
                    ...
                    <div id="img_overlay" class="absolute bottom-2 right-2 p-1 bg-zinc-900 rounded-md opacity-50 hover:opacity-100 transition ease-in-out duration-150 leading-none z-10">
                        <a target="_blank" href="{{ asset('storage/' . $hostedImage->path) }}" class="material-symbols-outlined w-6 h-6 leading-none flex items-center justify-center">zoom_out_map</a>
                    </div>

                    <img 
                        src="{{ asset('storage/' . $hostedImage->path) }}" 
                        alt="{{ $hostedImage->file_name }}" 
        ...
        @auth
                ...
                <div class="p-3 bg-indigo-400 col-start-1 sm:col-start-1 flex justify-between sm:row-start-1 sm:flex-col sm:justify-self-end h-min sm:justify-center sm:self-center rounded-md gap-1.5 align-center">
                    <p class="text-gray-800 text-center"><b>{{ $hostedImage->likes()->count() }}</b> likes</p>
                    <hr class="my-1.5 hidden sm:flex"/>
                    <h1 class="font-bold text-gray-900 hidden sm:flex">.like the image?</h1>
                    @if($user_id != $hostedImage->user_id)
                        @if($liked)
                            <form action="{{ route('uploads.dislike', $hostedImage->id) }}" method="POST" class="flex align-center justify-center">
                                @csrf
                                <button type="submit" class="material-symbols-outlined text-red-500 filled">
                                    favorite
                                </button>
                            </form>
                        @else
                            <form action="{{ route('uploads.like', $hostedImage->id) }}" method="POST" class="flex align-center justify-center">
                                @csrf
                                <button type="submit" class="material-symbols-outlined text-gray-800">
                                    favorite
                                </button>
                            </form>
                        @endif
        ...
        <div class="grid gap-3 p-6 w-full sm:p-6 justify-self-center rounded-md bg-zinc-800 col-start-1 sm:col-start-2">
            <div id="upload_details">
                <p>Size: {{ $hostedImage->file_size }} bytes - Type: {{ $hostedImage->file_type }}</p>
                <p>Uploaded by: {{ $hostedImage->user->name }}</p>
            </div>
            ...
            <div class="flex flex-col gap-3 w-full">
                ...
                @if (auth()->id() === $hostedImage->user_id || $is_admin)
                    ...
                    {{-- Edit button --}}
                    @if (!$editing)
                        <a href="{{ route('uploads.show', $hostedImage->id) }}?edit=1" class="cursor-pointer w-full h-full text-center py-1 bg-indigo-400 border border-transparent font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-300 rounded-md">Edit</a>
                    @else
                    <a href="{{ route('uploads.show', $hostedImage->id) }}" class="text-center cursor-pointer w-full h-full py-1 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-500 active:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition ease-in-out duration-150">Cancel editing</a>
                    @endif
